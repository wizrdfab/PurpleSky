...
from incremental_features import IncrementalBarAggregator
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare backtest results between full features and a live-style predictor replay."
    )
    parser.add_argument(
        "--model-dir",
        type=str,
...
def _summarize_bounce_diffs(full_probs: np.ndarray, live_probs: np.ndarray) -> None:
    diffs = np.abs(full_probs - live_probs)
...
def _simulate_live_features(
    trades: pd.DataFrame,
    predictor: TrendFollowerPredictor,
    timeframe_names: List[str],
    timeframes: List[int],
    base_tf: str,
    test_bar_times: List[int],
) -> pd.DataFrame:
    base_idx = timeframe_names.index(base_tf)
    tf_seconds = dict(zip(timeframe_names, timeframes))
    non_base_tfs = sorted(
        [tf for tf in timeframe_names if tf != base_tf],
        key=lambda name: tf_seconds[name],
    )

    aggregators = {
        tf_name: IncrementalBarAggregator(tf_seconds[tf_name])
        for tf_name in timeframe_names
    }

    test_times_set = set(int(x) for x in test_bar_times)
    collected = []

    ts_col = predictor.config.data.timestamp_col
    price_col = predictor.config.data.price_col
    size_col = predictor.config.data.size_col
    ...
    for idx, row in trades.iterrows():
        ...
        side = float(row.get("side_num", 0))
        tick_dir = float(row.get("tick_dir_num", 0))

        completed: Dict[str, Dict] = {}
        for tf_name, agg in aggregators.items():
            bar = agg.add_trade(timestamp, price, size, side, tick_dir)
            if bar is not None:
                completed[tf_name] = bar

        if completed:
            for tf_name in non_base_tfs:
                if tf_name in completed:
                    predictor.add_bar(tf_name, completed[tf_name])

            if base_tf in completed:
                features = predictor.add_bar(base_tf, completed[base_tf])
                bar_time = completed[base_tf].get("bar_time")
                if bar_time is not None and int(bar_time) in test_times_set:
                    collected.append(features)

        if idx > 0 and idx % 250000 == 0:
            print(f"  Processed {idx:,}/{total_trades:,} trades...")

    live_df = pd.DataFrame(collected)
    return live_df
...
def main() -> None:
    ...
    trades = trades.sort_values(cfg.data.timestamp_col)
    ...
    print("Simulating live incoming trades with predictor...")
    predictor = TrendFollowerPredictor(cfg, use_incremental=True, use_calibration=bool(args.use_calibration))
    predictor.load_models(model_dir)

    live_df = _simulate_live_features(
        trades,
        predictor,
        cfg.features.timeframe_names,
        cfg.features.timeframes,
        base_tf,
        test["bar_time"].tolist(),
    )

    live_aligned = pd.merge(test[["bar_time"]], live_df, on="bar_time", how="left")
    missing = int(live_aligned.isna().all(axis=1).sum())
    if missing:
        print(f"WARNING: {missing} test bars missing live features after replay.")
    print("")
    ...
    print("Running backtest (live predictor features)...")
    res_live = run_backtest(live_aligned)

    print("")
    _summarize_backtest("FULL", res_full)
    _summarize_backtest("LIVE", res_live)
    ...
    if common_keys:
        ...
        for key in common_keys:
            ...
            diffs.append(abs(float(t_full.bounce_prob) - float(t_live.bounce_prob)))
    ...
    X_live = _build_feature_matrix(live_aligned, feature_cols)

    pred_full = models.entry_model.predict(X_full, use_calibration=bool(args.use_calibration))
    pred_live = models.entry_model.predict(X_live, use_calibration=bool(args.use_calibration))

    full_probs = np.asarray(pred_full["bounce_prob"], dtype=float)
    live_probs = np.asarray(pred_live["bounce_prob"], dtype=float)

    if len(full_probs) != len(live_probs):
        print("WARNING: bounce_prob arrays differ in length.")
    else:
        _summarize_bounce_diffs(full_probs, live_probs)
